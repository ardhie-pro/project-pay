      ...
      <div class="content-wrapper full-page-wrapper d-flex align-items-center auth login-bg">
        <div class="card col-lg-4 mx-auto">
          <div class="card-body px-5 py-5">
            <h1 class="card-title text-center ">E-Nuqd</h1>
            ...
            <form method="POST" action="<?= base_url('/actionindex') ?>">
              <div class="form-group">
                <label>Your Card</label>
                <input type="password" id="nims" name="nis_siswa" placeholder="Scan Your Card" class="form-control p_input text-light" autofocus onkeyup="cekdatapengunjung()">
              </div>
              <div class="text-left">
...
<script type="text/javascript">
  function cekdatapengunjung() {
    var userVal = $("#nims").val();
    $.ajax({
      url: "<?= base_url('cekdatapengunjung') ?>/" + userVal,
      type: "POST",
      dataType: "JSON",
      success: function(data) {
        ...
        if (data != null) {
          ...
          $('[name="kelas"]').val(data.kelas).change();
          $('[name="rombel"]').val(data.rombel).change();
          $('[name="instansi"]').val(data.instansi).change();
          $('#klik').delay(1800).trigger('click');
        }
